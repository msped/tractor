...
class CaseModelTests(NetworkBlockerMixin, TestCase):
    def test_case_str(self):
        case = Case.objects.create(case_reference="202501", data_subject_name="Alice Example")
    ...
    def test_case_creation_with_user(self):
        ...
        case = Case.objects.create(case_reference="20251A", data_subject_name="User Test", created_by=user)
    ...
    def test_retention_review_date_for_adult(self):
        ...
        case = Case.objects.create(case_reference="202502", data_subject_name="Adult Example", data_subject_dob=dob)
    ...
    def test_calculate_retention_date_for_adult_direct(self):
        ...
        case = Case(case_reference="20252A", data_subject_name="Adult Direct", data_subject_dob=dob)
        case.retention_review_date = case._calculate_retention_date(today=fixed_today)
    ...
    def test_retention_review_date_for_minor(self):
        ...
        dob = date(2015, 9, 27)
        case = Case(case_reference="202503", data_subject_name="Minor Example", data_subject_dob=dob)
    ...
    def test_retention_review_date_for_eighteen_year_old(self):
        ...
        case = Case(case_reference="20253B", data_subject_name="18yo", data_subject_dob=dob)
    ...
    def test_retention_review_date_no_dob(self):
        ...
        case = Case.objects.create(case_reference="202504", data_subject_name="No DOB")
        expected = date(2031, 9, 27)  # 2025 + 6 years
        self.assertEqual(case.retention_review_date, expected)

    def test_case_status_default(self):
        case = Case.objects.create(case_reference="202505", data_subject_name="Status Default")
        self.assertEqual(case.status, Case.Status.OPEN)

    def test_case_export_status_default(self):
        case = Case.objects.create(case_reference="202506", data_subject_name="Export Status Default")
    ...
    def test_start_export_method(self, mock_async_task):
        ...
        case = Case.objects.create(case_reference="202507", data_subject_name="Export Method Test")
        self.assertEqual(case.export_status, Case.ExportStatus.NONE)

        task_id = case.start_export()

        self.assertEqual(task_id, "model-test-task-id")
        mock_async_task.assert_called_once_with("cases.services.export_case_documents", case.id)
...
class DocumentModelTests(NetworkBlockerMixin, TestCase):
    def setUp(self):
        self.case = Case.objects.create(case_reference="202510", data_subject_name="Doc Test")
    ...
    def test_document_str(self):
        ...
        doc = Document.objects.create(case=self.case, original_file=file, spacy_model=self.model)
        self.assertEqual("test.pdf", return_test_file_name(doc.original_file.name))
    ...
    def test_document_filename_and_filetype(self):
        ...
        self.assertEqual(return_test_file_name(doc.original_file.name), "sample.docx")
    ...
    def test_document_filename_and_filetype_on_save(self):
        ...
        doc = Document.objects.create(case=self.case, original_file=file)
        self.assertEqual(return_test_file_name(doc.original_file.name), "report.pdf")
...
class RedactionModelTests(NetworkBlockerMixin, TestCase):
    def setUp(self):
        self.case = Case.objects.create(case_reference="202520", data_subject_name="Redaction Test")
        file = SimpleUploadedFile("redact.txt", b"file_content")
        self.document = Document.objects.create(case=self.case, original_file=file)
    ...
    def test_redaction_with_justification(self):
        justification_text = "This is a manual redaction for a specific reason."
        redaction = Redaction.objects.create(
            document=self.document,
            start_char=0,
            end_char=10,
...
class RedactionContextModelTests(NetworkBlockerMixin, TestCase):
    def setUp(self):
        self.case = Case.objects.create(case_reference="202530", data_subject_name="Context Test")
        file = SimpleUploadedFile("context.txt", b"file_content")
        self.document = Document.objects.create(case=self.case, original_file=file)
        self.redaction = Redaction.objects.create(document=self.document, start_char=0, end_char=4, text="test")
