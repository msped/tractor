...
from ..extractors.gliner_extractor import (
    _MAX_CHUNK_CHARS,
    GLINER_LABELS,
    _chunk_text,
    extract_with_gliner,
...
class ExtractWithGLiNERTests(NetworkBlockerMixin, TestCase):
    ...
    def test_multiple_entities_all_third_party(self):
        ...
        results = extract_with_gliner(
            model, "SMITH, John lives in Manchester."
        )
    ...
    def test_calls_predict_entities_with_correct_labels(self):
        ...
        model.predict_entities.assert_called_once_with(
            "test text", GLINER_LABELS, flat_ner=True, threshold=0.35
        )
    ...
    def test_all_third_party_labels(self):
        ...
        for label in third_party_labels:
            ...
            self.assertEqual(
                results[0]["label"],
                "THIRD_PARTY",
                f"Label '{label}' should map to THIRD_PARTY",
            )
    ...
    def test_no_operational_labels_in_gliner(self):
        ...
        for label in operational_labels:
            self.assertNotIn(
                label,
                GLINER_LABELS,
                f"'{label}' should not be in GLINER_LABELS",
            )
    ...
    def test_long_text_is_chunked_and_offsets_applied(self):
        ...
        self.assertEqual(
            results[0]["end_char"], _MAX_CHUNK_CHARS + len(entity_text)
        )
